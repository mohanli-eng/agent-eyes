...
1. **PRD Author Subagent**:通过 `/draft-prd` 起独立 subagent(Clarifier 循环澄清 + Drafter 生成 PRD),主 agent 仅承担用户对话中介与落盘
...
6. **Reviewer Subagent**:4 个 `/review-*` 指令都通过 task tool 起独立 subagent,确保审核独立性、避免 self-evaluation bias
7. **PRD 严禁内容**:PRD 不允许包含 Implementation Phasing / 时间预算 / 具体步骤拆分(这些属于 /plan 阶段产出)
8. **Subagent 调用规范外置**:PRD Author 与 Reviewer 的详细规范分别置于 `references/subagents/prd-author.md` 与 `references/subagents/reviewer.md`
...
本 skill 使用两类独立 subagent,具体调用规范分别外置:

| Subagent 类型 | 调用方 | 规范文件 |
| --- | --- | --- |
| **PRD Author** | `/draft-prd` | `references/subagents/prd-author.md` |
| **Reviewer** | `/review-prd` / `/review-plan` / `/review-test-plan` / `/review-implement` | `references/subagents/reviewer.md` |

**核心约束**:所有 subagent 调用必须通过 task tool 起独立实例,主 agent 仅承担用户对话中介、结果落盘、workflow-state.json 维护职责。禁止主 agent 直接执行 subagent 应承担的工作。

具体的输入参数、prompt 模板、输出 JSON schema、失败处理见对应规范文件。
...
`/draft-prd` 的核心工作通过两类独立 subagent 完成,主 agent 仅承担用户对话中介与落盘职责。**详细的 prompt 模板、输入参数、输出 JSON schema、失败处理见 `references/subagents/prd-author.md`**。

#### 主 agent 工作流

```
1. 读取 references/prd-template.md
2. Clarification Loop(上限 5 轮):
   循环:
     a. 通过 task tool 起 Clarifier subagent
        输入: 需求草稿 + 模板 + 历史问答
     b. 接收 JSON: { parsedRequirement, openQuestions }
     c. 若 openQuestions 为空 → 退出 loop
        若已达上限 5 轮 → 警告用户后强制退出
        否则 → 渲染问题给用户、收集回答、追加到历史问答、继续 loop
3. 通过 task tool 起 Drafter subagent
   输入: 需求草稿 + 模板 + 最终 parsedRequirement + 全部历史问答
   接收: PRD markdown 全文
4. 主 agent 落盘:
   - 任务 ID {id} 由主 agent 从需求语义自动生成(kebab-case, 2-3 词)
   - 保存至 references/prd-{id}.md
5. 呈现给用户确认,根据反馈微调
6. 用户确认后更新 workflow-state.json:
   - currentStage → "draft-prd"
   - completedStages 追加 "draft-prd"
   - documents.prd → "references/prd-{id}.md"
7. 输出"下一步: 必须执行 /review-prd 对 PRD 进行规范性审核"
```

#### 关键约束

- **禁止主 agent 直接解析需求或生成 PRD**:必须经 Clarifier + Drafter
- **禁止跳过 Clarification Loop**:即使草稿看起来很完整,至少经过一次 Clarifier
- **历史问答必须完整传递**:Clarifier 是 stateless 的,每次调用都要带全部历史
- **遇到模糊必须问,不能猜**:Clarifier subagent 的核心行为原则
...
通过 task tool 起独立 Reviewer subagent 执行,**详细规范见 `references/subagents/reviewer.md`**。
...
每条 rubric 给出 **pass / fail / warning** 判定 + 具体证据。

**Manual Action Items 识别范围**:
...
- 审核对象:`references/prd-{id}.md` + `references/coding-plan-{id}.md` + `references/test-plan-{id}.md`
- 审核规范:`references/review-rubrics/test-plan.md`
- 项目上下文:项目根目录 `CLAUDE.md`(若存在)
...
### 5. PRD Author 与 Reviewer 必须通过 task tool 独立执行

`/draft-prd` 与所有 `/review-*` 必须通过 task tool 起 subagent 执行,避免主 agent context 污染、self-evaluation bias。详细调用规范见 `references/subagents/`。
