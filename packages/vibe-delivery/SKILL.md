...
## 功能需求
（每条需求用用户故事格式，附验收标准；异常验收标准与正向同等重要，必须显式列出）
...
**异常验收标准**（仅写本 US 特有的异常情况，跨 US 通用异常移到下方"通用异常处理"章节）：
- AC-01-E1: 给定 {异常输入条件}，返回 {具体结构化错误} `[→ TC-XX]`
- AC-01-E2: ... `[→ TC-XX]`

> `[→ TC-XX]` 为预留 testId 占位符，在 /test-plan 阶段填写对应测试用例编号，实现 AC → 测试用例的双向追溯。
> 异常 AC 编号格式：`AC-{US编号}-E{序号}`，与正向 AC 区分。
...
### NFR 验收标准
（每条 NFR 必须有可验证的 AC，不允许仅有口号式描述）
- NFR-AC-01: {可量化的非功能验证条件} `[→ TC-XX]`
- NFR-AC-02: ... `[→ TC-XX]`
...
## 通用异常处理
（仅放跨多个 US 共享的异常情况，避免在每个 US 中重复定义）

以下异常情况对所有 / 多个相关功能均适用：

1. **{异常类型 1}**：{触发条件} → {统一响应格式}
2. **{异常类型 2}**：... → ...

各 US 特有的异常情况已在对应 user story 的"异常验收标准"小节定义。
```

> **关键约束**：
> 1. 验收标准必须精确、无歧义、可被验证。写"用户可在 3 秒内看到结果"而非"响应要快"。
> 2. 每条正向 AC 必须包含「具体输入 → 具体可断言输出」的结构。例如不写"返回快照"，而写"返回包含 h1-h6 文本、所有 button 的 accessible name、所有 link 的 href 和 text 的快照"。
> 3. 异常场景禁止以独立的散表形式呈现。必须按以下规则归类：
>    - **特定于某 US 的异常** → 写到该 US 的"异常验收标准"小节
>    - **跨多个 US 共享的异常**（如 URL 不可达、超时、鉴权缺失等基础设施级异常）→ 写到"通用异常处理"章节
>    - 这样测试用例可以从每条 AC 直接派生，不会出现"边界条件表格里某行不知道对应哪个 US"的孤儿问题
> 4. NFR 必须配套 NFR-AC，否则该 NFR 视为不可验收，需要重新定义或删除。
